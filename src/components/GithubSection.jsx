...
const GithubSection = ({ lang }) => {
    const t = T[lang] || T.en
    return (
        <section id="github" className="relative z-10 section-pad w-full flex flex-col items-center">
            ...
            <div className="w-full max-w-6xl px-4 sm:px-6 flex flex-col items-center">
                ...
                <div className="text-center mb-8">
                    ...
                    <motion.p initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.16 }}
                        className="text-slate-500 max-w-md mx-auto text-base mt-4">{t.p}</motion.p>
                </div>
                        ...
                        <motion.div key={repo.name}
                            ...
                            <div className="flex items-start justify-between mb-2">
                                <div className="flex items-center gap-2">
                                    <FiGithub className="text-slate-500 text-base" />
                                    <span className="font-mono font-bold text-base text-white">{repo.name}</span>
                                </div>
                                <div className="flex items-center gap-1 text-sm text-yellow-400">
                                    <FiStar size={11} /> {repo.stars}
                                </div>
                            </div>
                            <p className="text-slate-500 text-sm mb-3">{repo.desc}</p>
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-1.5">
                                    <div className="w-2.5 h-2.5 rounded-full" style={{ background: repo.color }} />
                                    <span className="text-sm text-slate-500 font-mono">{repo.lang}</span>
                                </div>
                ...
                <div className="text-center">
                    <motion.a href="https://github.com/abdulaziz" target="_blank" rel="noreferrer" id="github-profile-cta"
                        ...
                        className="inline-flex items-center gap-3 px-10 py-4 rounded-2xl font-bold text-base text-white magnetic-btn tracking-wider uppercase transition-all duration-300"
